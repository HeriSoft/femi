...
app.post('/api/gemini/image/generate', async (req, res) => {
  try {
    ...
    if (req.isPaidUser) {
      ...
      // If not paid and not demo, deny access. This handles deleted users.
...
app.post('/api/openai/tts/generate', async (req, res) => {
  try {
    ...
    if (req.isPaidUser) {
      ...
      // If not paid and not demo, deny access.
...
app.post('/api/fal/image/generate/flux-ultra', async (req, res) => { 
  try {
    ...
    if (req.isPaidUser && req.paidUser) {
      ...
      // If not paid and not demo, deny access. This handles admin as well (admin must be logged in as a 'paid' type user for this)
